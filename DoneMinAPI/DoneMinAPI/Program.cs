...
app.MapPost("/User", (string email,string password) =>
{
    ...
    return Results.Created($"/employees/{u.GetUsername()}", u);
});
...
//get a user's approved/denied tickets
app.MapGet("/{user.GetUsername()}/tickets/approved={app}", (bool app) => {
    User u = new User("Leif@tree.com", "red");
    return repo.GetUserTickets(u, app);
});

//Employee=====================================================================================
app.MapPost("/{user.GetUsername()}/review", (User u, int app) =>
{
    User user = new User("strong@bad.com", "sb");
    Ticket t = repo.CreateTicket(user, "software", 2.00);
    repo.AddTicket(t);
    repo.ChangeTicketStatus(app);
    return Results.Created($"/{u.GetUsername()}/review", t);
});

//get all pending tickets
app.MapGet("/{user.GetUsername()}/tickets/pending", () => {
    User user = new User("A@B.Com", "Alphabet", true);
